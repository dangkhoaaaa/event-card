...
export default function Template2Page() {
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    try {
      // Template 2 là frontend-rendered, chỉ cần gửi tên template
      // Backend sẽ tự động tạo template record nếu chưa có
      const templateId = 'template2'; // Backend sẽ tự động tạo Template 2 nếu chưa có

      // Chuẩn bị content - tất cả là text, images sẽ được upload riêng
      ...
      // Thêm image keys vào content để backend biết cần map với ảnh nào
      // Các image keys này sẽ được thay thế bằng URL sau khi upload
      Object.keys(images).forEach((imageKey) => {
        if (!content[imageKey]) {
          // Thêm placeholder để backend biết đây là image field
          content[imageKey] = '__IMAGE_PLACEHOLDER__';
        }
      });

      // Gọi API để tạo card với images
      ...
      router.push(`/templates/template2/${result._id}`);
    } catch (error: any) {
        ...
        <div className="fixed top-0 right-0 w-96 h-full bg-white border-l-2 border-dark-200 shadow-2xl z-50 overflow-y-auto p-6">
          ...
          <div className="space-y-4">
            ...
            <div className="mt-8 pt-6 border-t border-dark-200">
              ...
              >
                {isSaving ? 'Đang lưu...' : '💾 Lưu thiệp'}
              </button>
              
              {savedCard && (
                <div className="mt-4 p-4 bg-green-50 rounded-lg">
                  <p className="text-sm font-semibold text-green-800 mb-2">Đã lưu thành công!</p>
        ...
        <section className="min-h-screen bg-gradient-to-br from-pink-50 to-purple-50 flex items-center justify-center p-4 overflow-hidden relative">
          ...
          <div className="max-w-md w-full text-center relative z-10">
            ...
            >
              ...
              >
                {imagePreviews['envelopePhoto'] ? (
                  <div className="relative w-full h-full">
                    <Image
                      src={imagePreviews['envelopePhoto']}
                      alt="Envelope photo"
                      fill
                      className="object-cover"
          ...
          <div className="max-w-md w-full relative z-10">
            ...
            <AnimatedImage animationType="scale" delay={200}>
              <div className="relative w-full h-96 mb-6 border-4 border-red-700 rounded-lg overflow-hidden">
                {imagePreviews['mainPhoto'] ? (
                  <div className="relative w-full h-full">
                    <Image
                      src={imagePreviews['mainPhoto']}
                      alt="Main photo"
                      fill
                      className="object-cover"
                      sizes="(max-width: 768px) 100vw, 400px"
                ...
                return (
                  <AnimatedImage key={i} animationType="zoomIn" delay={400 + i * 100}>
                    <div className="relative aspect-square bg-red-700 border-2 border-red-800 rounded-lg overflow-hidden">
                      {imagePreviews[photoKey] ? (
                        <div className="relative w-full h-full">
                          <Image
                            src={imagePreviews[photoKey]}
                            alt={`Photo ${i}`}
                            fill
                            className="object-cover"
            ...
            {/* Calendar */}
            <WeddingCalendar weddingDate={formData.weddingDate} weddingYear={formData.weddingYear} />
          </div>
        </section>
          ...
          <div className="max-w-md w-full text-center relative z-10">
            ...
            <AnimatedImage animationType="scale" delay={400}>
              <div className="relative w-full h-96 mb-8 rounded-lg overflow-hidden shadow-2xl">
                {imagePreviews['thankMainPhoto'] ? (
                  <div className="relative w-full h-full">
                    <Image
                      src={imagePreviews['thankMainPhoto']}
